# -*- coding: utf-8 -*-
"""
Created on Wed Apr 10 15:22:36 2019
...
class GAList(object):
    ...
    def initPopulation(self):
        ...
        while unitCount>0:
            bestgene = [x for x in range(self.geneLenght)]
            random.shuffle(bestgene)              # 随机洗牌 #
            bestunit = GAUnit(bestgene)
            bestunit.value=self.matchFun(bestunit)
            for i in range(10):                
                gene = [x for x in range(self.geneLenght)]
                random.shuffle(gene)              # 随机洗牌 #
                unit = GAUnit(gene)
                unit.value=self.matchFun(unit)
                if unit.value>bestunit.value:
                    bestunit=unit
            self.population.append(bestunit)
            unitCount-=1
