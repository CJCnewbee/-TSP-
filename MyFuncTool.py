...
def ResultShow(Min_Path,BestPath,CityNum,string):
    """
        ...
        其他说明：无
    """
    strans="基于"+string+"求得的旅行商最短路径为:"
    strans +='\n'
    for m in range(CityNum):
        strans+=str(BestPath[m])+"—>"
    strans+=str(BestPath[CityNum])
    strans += '\n'
    strans+="总路径长为："+str(Min_Path)
    strans += '\n'
    return strans
...
#     plt.show()
#
# def drawt(distance_list,title):
#     """
#         函数名：draw(BestPath,Position,title)
#         函数功能：    表现迭代中距离的变化
#             输入    1     distance_list：距离表
#             输出    无
#         其他说明：无
#     """
#     plt.title(title)
#     #for i in range(len(distance_list)):
#     plt.plot(range(len(distance_list)),distance_list,'red')
#     plt.show()
